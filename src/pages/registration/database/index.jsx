...
const RegistrationDatabase = () => {
    ...
    const handleSubmit = (idButton) => {
        switch (idButton) {
            case "btn-save":
                if (!structure || !client || !brands || !server || !instance || !charset || !dbUser || !size || !dtaDmpBak) {
                    ...
                    handleDatabaseEdit(dbID, structure, client, brands, server, instance, charset, dbUser, size, dtaDmpBak)
                }
                break
            case "btn-insert":
                if (!structure || !client || !brands || !server || !instance || !charset || !dbUser || !size || !dtaDmpBak) {
                    ...
                    handleDatabaseRegistration(structure, client, brands, server, instance, charset, dbUser, size, dtaDmpBak)
                }
                break
            case "btn-cancel":
            ...
            case "btn-delete":
                if (!dbID) {
                    warningMessage("Selecione uma base de dados para deletar", "Aviso")
                } else {
                    hanldeDatabaseDelete(dbID)
                }
                break
        }
    }

    const handleDatabaseRegistration = async (structure, client, brands, server, instance, charset, dbUser, size, dtaDmpBak) => {
        setIsLoading(true)
        await databaseRegistration(structure, client, brands, server, instance, charset, dbUser, size, dtaDmpBak)
            .then((resp) => {
    ...
    const handleDatabaseEdit = async (dbID, structure, client, brands, server, instance, charset, dbUser, size, dtaDmpBak) => {
        setIsLoading(true)
        await databaseEdit(dbID, structure, client, brands, server, instance, charset, dbUser, size, dtaDmpBak)
            .then((resp) => {
                successMessage(resp.data.message, "Sucesso")
                clearImputs()
            }).catch((err) => {
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="col-5">
                                            <div className="form-group form-group-registration mb-4">
                                                ...
                                                <label className="form-label form-label-registration">Conjunto de Caracteres</label>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="row">
